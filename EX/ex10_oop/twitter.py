...
def find_fastest_growing(tweets: list) -> Tweet:
    ...
    ratio_list = []
    for tweet in tweets:
        ratio_list.append(round(tweet.retweets / tweet.time, 3))
    for tweet in tweets:
        if round(tweet.retweets / tweet.time, 3) == max(ratio_list):
            return tweet
...
def sort_hashtags_by_popularity(tweets: list) -> list:
    ...
    for tweet in tweets:
        if "#" in tweet.content:
            for match in re.finditer(r"#\S+", tweet.content):
        ...
        for tweet in tweets:
            if hashtag in tweet.content:
                if hashtag not in hashtag_dict:
                    ...
                    hashtag_dict[hashtag] += tweet.retweets
    return sorted(hashtags, key=lambda x: hashtag_dict[x], reverse=True)
