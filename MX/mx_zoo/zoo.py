...
def find_smallest_animal_by_weight(animal_list: list) -> Animal:
    ...
    """
    return list(filter(lambda x: x.weight_range[0] == min(animal_list, key=lambda y: y.weight_range[0]).weight_range[0],
                       animal_list))[0]
...
def find_how_many_pumpkins_are_needed_to_feed_animals(animal_list: list) -> int:
    ...
    """
    return math.ceil(sum(list(map(lambda x: sum(x.weight_range) / 2 * 0.06 / 3, list(filter(lambda x: x.diet != "carnivorous", animal_list))))) * 90)
